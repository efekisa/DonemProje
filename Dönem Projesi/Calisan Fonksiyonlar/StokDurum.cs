    ...
    public partial class StokDurum : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (urun == "Kalem")
            {
                ...
                {
                    var tur = Urunler.kalemler.FirstOrDefault(k => k.UrunKodu == kod);
            ...
            else if (urun == "Masa")
            {
                if (int.Parse(textBox1.Text) > 0)
                {
                    var tur = Urunler.masalar.FirstOrDefault(k => k.UrunKodu == kod);
                    if (tur != null) { tur.Adet = int.Parse(textBox1.Text); }
                    MessageBox.Show("Stok başarıyla güncellenmiştir.");
                }
                else
            ...
            else if (urun == "Sandalye")
            {
                if (int.Parse(textBox1.Text) > 0)
                {
                    var tur = Urunler.sandalyeler.FirstOrDefault(k => k.UrunKodu == kod);
                    if (tur != null) { tur.Adet = int.Parse(textBox1.Text); }
                    MessageBox.Show("Stok başarıyla güncellenmiştir.");
                }
                else
            ...
            else if (urun == "Kalemlik")
            {
                if (int.Parse(textBox1.Text) > 0)
                {
                    var tur = Urunler.kalemlikler.FirstOrDefault(k => k.UrunKodu == kod);
                    if (tur != null) { tur.Adet = int.Parse(textBox1.Text); }
                    MessageBox.Show("Stok başarıyla güncellenmiştir.");
                }
                else
            ...
            else if (urun == "Pano")
            {
                if (int.Parse(textBox1.Text) > 0)
                {
                    var tur = Urunler.panolar.FirstOrDefault(k => k.UrunKodu == kod);
                    if (tur != null) { tur.Adet = int.Parse(textBox1.Text); }
                    MessageBox.Show("Stok başarıyla güncellenmiştir.");
                }
                else
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            var bulunanKalemler = Urunler.kalemler.Where(k => k.UrunKodu.ToLower().Contains(aranan)).ToList();
            foreach (var k in bulunanKalemler)
            {
                listBox1.Items.Add($"Kalem {k.UrunKodu} - Stok: {k.Adet}");
            }

            var bulunanMasalar = Urunler.masalar.Where(m => m.UrunKodu.ToLower().Contains(aranan)).ToList();
            foreach (var m in bulunanMasalar)
            {
                listBox1.Items.Add($"Masa {m.UrunKodu} - Stok: {m.Adet}");
            }

            var bulunanSandalyeler = Urunler.sandalyeler.Where(d => d.UrunKodu.ToLower().Contains(aranan)).ToList();
            foreach (var d in bulunanSandalyeler)
            {
                listBox1.Items.Add($"Sandalye {d.UrunKodu} - Stok: {d.Adet}");
            }

            var bulunanKalemlikler = Urunler.kalemlikler.Where(d => d.UrunKodu.ToLower().Contains(aranan)).ToList();
            foreach (var d in bulunanKalemlikler)
            {
                listBox1.Items.Add($"Kalemlik {d.UrunKodu} - Stok: {d.Adet}");
            }

            var bulunanPanolar = Urunler.panolar.Where(d => d.UrunKodu.ToLower().Contains(aranan)).ToList();
